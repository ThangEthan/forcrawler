#!/bin/zsh
# This script scrape WOZ site for house coordinate.
RED='\033[0;31m'
NC='\033[0m'

function req(){
    # Search address line to receive address id. Use Network tab in Chrom DevTools to make this curl request
    ...
    # Use address id to retrive x y coordinate. 
...
done < <(mysql --login-path=server -s -N scraping_data -e "select id, URLENCODERCUR(concat(adres , ', ', postcode)) as line from funda where x is null and y is null order by id desc limit 50000;") # Prepare data to search
